import { GeneralSettings } from '../types/GeneralSettingsType';

export const defaultGeneralSettings: GeneralSettings = [
  {
    settingsCategoryName: 'General',
    settings: [
      {
        key: 'confirmOnInputSMILES',
        label:
          'Show confirmation before prediction if input bar contains a SMILES',
        type: 'boolean',
        default: true,
        value: true,
      },
      {
        key: 'confirmOnClear',
        label: 'Show confirmation before clearing the current project',
        type: 'boolean',
        default: true,
        value: true,
      },
      {
        key: 'predict1HAnd13COnPredict',
        label:
          'Automatically predict both 1H and 13C NMR when predicting a spectrum',
        type: 'boolean',
        default: true,
        value: true,
      },
    ],
  },
];
